...
class OsmThreadedDataWriter(DataWriterBase):
    ...
    def __converter_thread(self, geoms, thread_number):
        for index, osm_geom in enumerate(geoms):
            if index % AMOUNT_THREADS == thread_number:
                self.q.put(osm_geom.to_xml(self.attributes, \
    ...
    def __writer_thread(self):
        while True:
            try:
                geom_xml = self.q.get_nowait()
                with self.lock:
                    self.f.write(geom_xml)
                    self.f.write('\n')
                self.q.task_done()
            except queue.Empty:
    ...
    def __write_geometries(self, geoms):
        # start writer thread
        write_thread = threading.Thread(target=self.__writer_thread)
        self.conversion_finished = False
        write_thread.start()

        # start converter threads
        conv_threads = [ threading.Thread(target=self.__converter_thread, args=(geoms,i)) \
                            for i in range(AMOUNT_THREADS) ]
        for thread in conv_threads:
            thread.start()

        # wait for converer threads to finish
        for thread in conv_threads:
            thread.join()

        # block until all write tasks are finished and end the thread
